...
#include "sobits_interfaces/srv/moveit_to_tf.hpp"
#include "interfaces.hpp"

using MoveitToTF = sobits_interfaces::srv::MoveitToTF;
...
public:
  ...
  : Node("moveit_ik_service_node")
  {
    ...
    service_ = this->create_service<MoveitToTF>(
        "moveit_to_tf",
...
private:
  void callback(
      const std::shared_ptr<MoveitToTF::Request> request,
      std::shared_ptr<MoveitToTF::Response> response)
  {
    ...
    try
    {
      ...
      target_pose.position.x = transform.transform.translation.x - 0.20;
      target_pose.position.y = transform.transform.translation.y;
      target_pose.position.z = transform.transform.translation.z + 0.05;
  ...
  rclcpp::Service<MoveitToTF>::SharedPtr service_;
